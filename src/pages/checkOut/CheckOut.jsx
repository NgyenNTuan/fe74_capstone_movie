            ...
            <div className='grid grid-cols-12 mt-2'>
                <div className='col-span-9'>
                    <div className='flex flex-col items-center mt-5'>
                        <div className='linner' style={{ width: '100%', height: '20px' }}>
                ...
                <div className='col-span-3 ml-2 text-white '>
                    <div className='box__bill  p-4 border-spacing-2'>
                        ...
                        <div className='flex flex-row my-5'>
                                ...
                                <span className='text-red-400 font-bold'>Ghế:</span>
                                {
                                    _.sortBy(danhsachGheDangDat, ['stt']).map((gheDD, index) => {
                                        ...
                                        return <Fragment key={index}>
                                            ...
                                            <span key={index} className='text-orange-300 font-bold text-sm mx-0.5'>{gheDD.stt}</span>
                                            {(index + 1) % 6 === 0 ? <br /> : ''}
                                        </Fragment>
                            ...
                            <div className='text-right col-span-1'><span className='text-white text-sm'>
                                {
                                    danhsachGheDangDat.reduce((tongTien, ghe, index) => { return tongTien += ghe.giaVe }, 0).toLocaleString()
                                }đ
                        ...
                        <div className='mb-0 h-full flex flex-col '>
                            <div className='ticked1 cursor-pointer  text-white w-full text-center py-1  text-1xl my-1'
                                onClick={() => {
                                    const thongTinDatVe = {};
...
const KetQuaDatVe = () => {
    ...
    const dispatch = useDispatch()
    const { thongTinNguoiDung, user } = useSelector((state) => state.quanLyNguoiDung)
    useEffect(() => {
                ...
                <div className="p-2 lg:w-1/3 md:w-1/2 w-full " key={index}>
                    <div className="h-full flex items-center bg-r01 border-gray-200 border p-4 rounded-lg">
                        ...
                        <div className="flex-grow ">
                            <h2 className="text-white title-font font-medium">{ticked.tenPhim}</h2>
                            <p className="text-white">Giờ chiếu: {moment(ticked.ngatDat).format('hh:mm A')} - Ngày chiếu {moment(ticked.ngatDat).format('DD-MM-YYYY')}</p>
                            <p className="text-white">Địa điểm: {seats.tenHeThongRap}</p>
                            <p className="text-white">Tên rập: {seats.tenCumRap} - Ghế: <span className='letter text-orange-100'>{ghe}</span></p>
                        </div>
                    </div>
                </div>
            )
            ...
            <section className="text-gray-600 body-font">
                <div className="container px-1 py-1 mx-auto">
                    <div className="flex flex-col text-center w-full mb-5">
                        <h1 className="sm:text-3xl text-2xl font-medium title-font text-white">Lịch sử đặt vé</h1>
                        <p className="lg:w-2/3 mx-auto leading-relaxed text-base text-white">No hope.</p>
                    </div>
